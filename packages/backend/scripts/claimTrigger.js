...
async function claim (contract, beneficiary, claimee) {
  try {
    ...
    if (beneficiary.eq(claimee)) {
      ...
      gas = await contract.estimateGas.claimToRef(beneficiary, claimee)
    }
    ...
    if (fee.gt(txCost)) {
      ...
      if (beneficiary.eq(claimee)) {
        ...
        tx = await contract.claimToRef(beneficiary, claimee)
      }
...
async function triggerClaims (contract, tokenId, pendingPromise) {
  ...
  const references = await contract.getReferences(tokenId)
  if (references) {
